...
fit1 = SimpleExpSmoothing(np.array(train['Value'])).fit(smoothing_level=0.9, optimized=False)
currency_moving_avg['Value'] = fit1.forecast(len(test))
currency_moving_avg.to_csv('exponential_average.csv', index=0)
...
plt.ylabel('Currency')
plt.legend(loc='best')
plt.savefig('exponential_average.jpg', bbox_inches='tight')
plt.show()
