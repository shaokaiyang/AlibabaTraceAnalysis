...
# 绘制两组数据的CDF图像在一个图片里
# 读取数据文件,并将数据文件转化为数组
df = pd.read_csv('task_submit_currency_minute_all.csv', delimiter=",", skiprows=0)
...
line1, = plt.plot(x1, y1, label='Task_Submit_Per_Minute', linewidth=1)
...
plt.xlabel('Task Submit Number Per Minute')
# plt.axvline(1500, -1, 2, color='r', label='nim')
plt.savefig('taskperminute_cdf.jpg', bbox_inches='tight')
plt.show()
